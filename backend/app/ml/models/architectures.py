...
def build_lstm_model(input_shape: tuple, units: int = 128) -> tf.keras.Model:
...
def build_gru_model(input_shape: tuple, units: int = 128) -> tf.keras.Model:
    model = models.Sequential([
        layers.Input(shape=input_shape),
...
def build_cnn_pattern_model(input_shape: tuple, num_classes: int = 6) -> tf.keras.Model:
